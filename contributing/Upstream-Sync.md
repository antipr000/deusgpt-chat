...
Upgrading the Docker deployment version is very simple, just redeploy the latest image of DeusGPT Chat. Here are the instructions to perform these steps:

1. Stop and delete the currently running DeusGPT Chat container (assuming the name of the DeusGPT Chat container is `lobe-chat`):
...
2. Pull the latest Docker image of DeusGPT Chat:

```fish
docker pull lobehub/lobe-chat
```

3. Redeploy the DeusGPT Chat container using the newly pulled image:
...
>
> Don't worry, all of DeusGPT Chat's chat history is stored in your local browser. Therefore, when you redeploy DeusGPT Chat using Docker, your chat history will not be lost.

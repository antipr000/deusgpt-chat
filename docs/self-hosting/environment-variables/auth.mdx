---
title: DeusGPT Chat Authentication Service Environment Variables
description: >-
  Explore the essential environment variables for configuring authentication services in DeusGPT Chat, including OAuth SSO, NextAuth settings, and provider-specific details.
...
DeusGPT Chat provides a complete authentication service capability when deployed. The following are the relevant environment variables. You can use these environment variables to easily define the identity verification services that need to be enabled in DeusGPT Chat.

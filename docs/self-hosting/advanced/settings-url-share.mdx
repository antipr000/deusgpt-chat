...
DeusGPT Chat support import settings from external URL to quickly set up DeusGPT Chat configuration.
...
<Callout type={'warning'}>
  DeusGPT Chat does not verify the correctness of the settings parameters in the URL, nor provide
...
| Property name | Type | Description | | accessKeyId | string | The access key ID for AWS Bedrock. | | region | string | The region for AWS Bedrock. | | secretAccessKey | string | The secret access key for AWS Bedrock. |

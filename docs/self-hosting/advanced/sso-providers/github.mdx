---
title: Configuring Github Authentication Service for DeusGPT Chat
description: >-
  Learn how to configure Github authentication service for DeusGPT Chat, including creating a Github provider, setting up environment variables, and deploying DeusGPT Chat.
...
| --- | --- | --- |
| `ENABLE_OAUTH_SSO` | Required | Enable Single Sign-On (SSO) for DeusGPT Chat. Set to `1` to enable SSO. |
...
| `SSO_PROVIDERS` | Required | Select the Single Sign-On provider for DeusGPT Chat. Use `github` for Github. |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate with Github and use DeusGPT Chat.
</Callout>

---
title: Customizing Provider Model List in DeusGPT Chat for Deployment
description: >-
  Learn how to customize the model list in DeusGPT Chat for deployment with the syntax and extension capabilities
...
DeusGPT Chat supports customizing the model list during deployment. You can use `+` to add a model, `-` to hide a model, and use `model name=display name<extension configuration>` to customize the display name of a model, separated by English commas. The basic syntax is as follows:

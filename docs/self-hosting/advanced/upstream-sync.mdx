---
title: DeusGPT Chat Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for DeusGPT Chat Vercel deployments and upgrade Docker deployment versions easily. Follow step-by-step instructions for a seamless deployment process.
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest DeusGPT Chat image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running DeusGPT Chat Container

Assuming the DeusGPT Chat container is named `lobe-chat`, use the following commands to stop and remove the currently running DeusGPT Chat container:
...
### Pull the Latest DeusGPT Chat Image

Use the following command to pull the latest Docker image for DeusGPT Chat:
...
Redeploy the DeusGPT Chat container using the newly pulled image:
...
No need to worry, you won't. All of DeusGPT Chat's chat records are stored in your local browser. Therefore, when redeploying DeusGPT Chat using Docker, your chat records will not be lost.
